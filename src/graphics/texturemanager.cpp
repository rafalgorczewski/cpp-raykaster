...
namespace rk {
  bool TextureManager::load_textures(std::string textures_configuration_path) {
  ...
  void TextureManager::load_texture(std::string name, std::string path) {
    if (auto texture = sf::Texture(); !texture.loadFromFile(path)) {
      ...
      spdlog::error(
        "Texture Manager: Could not load texture: '{}' from: '{}'", name, path);
    }
  }
  ...
  sf::Texture& TextureManager::get_texture(std::string name) & {
    ...
    spdlog::error("Texture Manager: Trying to use unknown texture: '{}'", name);
