...
namespace rk {
  ...
  bool SpriteController::add_animation(const std::string& name,
    ...
    if (animation_already_exists(name)) {
      spdlog::error(
        "Sprite Controller: There is already an animation called: '{}'", name);
      return false;
    }
  ...
  void SpriteController::update() {
    if (!is_normal_animation_set()) {
      spdlog::error("Sprite Controller: No animation was set before update");
    } else {
  ...
  void SpriteController::select_as_immediate_animation(
    ...
    m_sprite.setTextureRect(
      m_animations[*m_selected_immediate_animation_name].get_current_tile());
  }
  ...
  void SpriteController::proceed_immediate_animation() {
    m_animations[*m_selected_immediate_animation_name].change_tile_to_next();
    m_sprite.setTextureRect(
      m_animations[*m_selected_immediate_animation_name].get_current_tile());
  }
  ...
  sf::Int32 SpriteController::elapsed_time() const {
